...
<div class="pdfHeaderEport hide">
	...
	@if($timePeriod == "C")
		...
		@elseif($timePeriod == "Y")
	 	<div class="timePeriodDetails">
				 Year: {{date("Y",strtotime($pdfArray['start_date']))}}
		</div>
	@endif
...
<div class="genderReportsDiv hide">
	...
	<div class="genderReports gender_report_1_M" data-type="M" data-index="1">SA: Boys: {{$selfAwarenessMale}}</div>
	<div class="genderReports gender_report_1_F" data-type="F" data-index="1">Girls: {{$selfAwarenessFemale}}</div>

	<div class="genderReports gender_report_2_M" data-type="M" data-index="2">SM: Boys: {{$selfManagementMale}}</div>
	<div class="genderReports gender_report_2_F" data-type="F" data-index="2">Girls: {{$selfManagementFemale}}</div>

	<div class="genderReports gender_report_3_M" data-type="M" data-index="3">RDM: Boys: {{$decisionMakingMale}}</div>
	<div class="genderReports gender_report_3_F" data-type="F" data-index="3">Girls: {{$decisionMakingFemale}}</div>

	<div class="genderReports gender_report_4_M" data-type="M" data-index="4">RS: Boys: {{$relationshipSkillsMale}}</div>
	<div class="genderReports gender_report_4_F" data-type="F" data-index="4">Girls: {{$relationshipSkillsFemale}}</div>

	<div class="genderReports gender_report_5_M" data-type="M" data-index="5">SA: Boys: {{$socialAwarenessMale}}</div>
...
@endphp
	<div class="behavioursReports behaviour_report{{$startBehavior}}">{{$key}}: {{$behaviourForPdf}}</div>
@php
	$startBehavior++;
	}
@endphp
...
<div class="locationReportsDiv hide">
	...
	foreach($LocationsShow as $key => $locationReportsForPdf){
		@endphp
			<div class="locationsReports location_report{{$startLocation}}">{{$key}}: {{$locationReportsForPdf}}</div>
		@php
			$startLocation++;
	}
	@endphp
...
@php
	...
	foreach($InterventionssShow as $key => $interventionReportsForPdf){
		@endphp
			<div class="interventionReports intervention_report{{$startIntervention}}">{{$key}}: {{$interventionReportsForPdf}}</div>
		@php
			$startIntervention++;
	}
	@endphp
	  ...
	  <div class="col-md-12">
		<div class="top_filter">
				...
				<form class="filter_resilts" @if(isset($_GET['duration']) && $_GET['duration'] == "C") validate @else novalidate @endif >
					...
					<div class="custom_filter_inputs @if(isset($_GET['duration']) && $_GET['duration'] == 'C') show_fields @else hide @endif">
						<div class="filter_input">
...
function headerTextPdf(className, x, y,type=""){
  ...
  if(type == "header"){
  	return '<text x= "' +x+ '" y = "' + y + '" style = "width:600px; height:50px;font-size: 12px;font-weight:bold">'+dataHeader+'</text>';
  }else{
  	return '<text x= "' +x+ '" y = "' + y + '" style = "width:600px; height:50px;font-size: 12px;">'+dataHeader+'</text>';
  }
...
$(function() {
  Highcharts.getSVG = function(charts, texts) {
    var svgArr = [],
      ...
      /*PDF HEADINGS--*/
var pdfMainHeader =  '<text x= "250" y = "'+top+'" style = "width:600px; height:50px;font-size: 13px;font-weight:bold">Analytics Report</text>';
...
/*Charts Begin--*/
    ...
    return '<svg height="' + (top+100) + '" width="' + width + '" version="1.1" xmlns="http://www.w3.org/2000/svg">' + svgArr.join('') + '</svg>';
  };
...
function checkDifferenceBetweenDate(){
 ...
 var minDate = moment();
 var maxDate = minDate.add(1, 'months');
