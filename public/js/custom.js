	...
	/*Charts Begin--*/
	    Highcharts.each(charts, function(chart, i) {
	      ...
	      if(i == 0){
			...
			/*PDF HEADINGS--*/
		    if($(".notes_for_pdf").val().length){
				var NotesTextFooter; var notesAdded;
				NotesTextFooter += '<text x= "20" y = "' + (top+50) + '" style = "width:600px; height:50px;font-size: 10px;font-weight:bold;">Notes</text>';/*Footer Notes*/
				svgArr.push(NotesTextFooter);
				top += 50;
				var addedNotes = $(".notes_for_pdf").val();
				var yourString = addedNotes;
				var charatcersToPrint = 100;


				var totalWords = addedNotes.split(' ').length;
				var wordsPerLine = 20;
				var totalLInes = totalWords/wordsPerLine;
				
				for(var start=0; start<totalLInes; start++){
					var notesAdded="";
					var startWord = start*wordsPerLine;
					var endWord   = ((start*wordsPerLine)+wordsPerLine);

					var str100 = addedNotes.split(' ').slice(startWord,endWord).join(' ');
					top = top+20;
					notesAdded += '<text x= "20" y = "' + top + '" style = "width:600px; height:50px;font-size: 10px;">'+str100+'</text>';/*Footer Notes*/ 
					svgArr.push(notesAdded);
				}
			}
	        $(".add_notespopup").modal("hide"); $(".add_notespopup textarea").val('');
